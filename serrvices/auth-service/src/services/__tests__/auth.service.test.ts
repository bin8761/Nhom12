...
import { RateLimitExceededError, InvalidCredentialsError, TokenExpiredError } from '../../utils/errors';
import { resendPhoneVerification, verifyPhone } from '../auth.service';
...
jest.mock('../../metrics/phoneVerificationMetrics', () => ({
  recordPhoneOtpFailed: (...args: unknown[]) => recordPhoneOtpFailedMock(...args),
  recordPhoneOtpVerified: (...args: unknown[]) => recordPhoneOtpVerifiedMock(...args),
}));
...
describe('auth.service phone verification', () => {
  ...
  describe('resendPhoneVerification', () => {
    ...
    it('enqueues OTP when user is eligible', async () => {
      ...
      await expect(
        resendPhoneVerification({ phoneNumber }, context),
  ...
  describe('verifyPhone', () => {
    ...
    it('verifies phone and clears record when code matches', async () => {
      await expect(
        verifyPhone({ phoneNumber, code }, context),
      ).resolves.toBeUndefined();
