...
export async function sendCandidateCvDecisionEmail(params: {
  ...
  const noteText = params.note ? `<br><br><strong>Ghi chú:</strong> ${params.note}` : '';
...
${approved 
  ? `Nhà tuyển dụng đã phê duyệt CV của bạn cho vị trí "${params.jobTitle}".${params.note ? `\n\nGhi chú: ${params.note}` : ''}\n\nChúng tôi sẽ liên hệ nếu có bước tiếp theo.`
  : `Nhà tuyển dụng chưa thể phê duyệt CV của bạn cho vị trí "${params.jobTitle}".${params.note ? `\n\nGhi chú: ${params.note}` : ''}\n\nBạn có thể cập nhật hồ sơ và ứng tuyển lại khi sẵn sàng.`
}
...
Bạn có một ứng viên mới ứng tuyển vào vị trí "${params.jobTitle}".

Thông tin ứng viên:
${params.candidateName ? `- Họ tên: ${params.candidateName}` : ''}
${params.candidateEmail ? `- Email: ${params.candidateEmail}` : ''}
${params.candidatePhone ? `- Số điện thoại: ${params.candidatePhone}` : ''}
...
Chúng tôi xin thông báo rằng tin tuyển dụng "${params.jobTitle}" mà bạn đã ứng tuyển đã bị xóa.
${params.reason ? `\nLý do: ${params.reason}` : ''}
